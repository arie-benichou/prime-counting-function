class PrimesCounterTest extends munit.FunSuite {
  ...
  for (i <- 10 to 2357) {
    test(s"consistency for x = $i") {
      assertEquals(PrimesCounter(1, PrimesCounter(i)), i.toLong)
    }
  }

  test("consistency for x = 105080512") {
    assertEquals(
      PrimesCounter(1, PrimesCounter(105080512)),
      105080512L
    )
  }
